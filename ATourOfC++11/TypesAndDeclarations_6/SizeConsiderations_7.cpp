 ...
 * change some features from one version to another. Hence, want to consider the impact of size
 * of variables.
 * 
 ...
 * 
 * Use of limits and size of help us determine everything
 * 
 * If  a  value v can  be  represented  exactly  in  a  variable  of  type T, 
 * a conversion  of v to T is  value-preserving.  
 * Conversions that are not value-preserving are best avoided.
 * 
 * The standard header<cstddef> defines an alias that is very widely used in both standard-library dec-larations 
 * and user code: size_t is an implementation-defined unsigned integer type that can hold the size  in  bytes  of  every  object. 
 * Consequently, it is used  where  we  need  to  hold  an  object  size.  For example: void* allocate(size_t n); // get n bytess
 */
#include <limits>
#include <iostream>
