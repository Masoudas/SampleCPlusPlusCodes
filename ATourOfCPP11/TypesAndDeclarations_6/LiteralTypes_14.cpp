 ...
 * 5- A class that has a trivial destructor, and one or more constexpr constructors that are not move or copy constructors. 
 * Additionally, all its non-static data members and base classes must be literal types and not volatile.
 * 
 * The reason why literal types are important is because "A constexpr variable or function must return a literal type."
 * Essentially, these are the stuff that we can assign to a constexpr.
 * 
 * 
 */
constexpr int y = 5;
